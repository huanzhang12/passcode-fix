...
static void solve_l2r_l1l2_svc(
	...
	double Cp, double Cn, int solver_type, int initB, int inner_iter)
{
	int l = prob->l;
	int w_size = prob->n;
	int i, s, iter = 0;
...
#pragma omp parallel for private(s,i) schedule(static)
			...
			//			xvtime += (double)(now.tv_sec+now.tv_nsec*1e-9) - (double)(start.tv_sec+start.tv_nsec*1e-9);
			...
			//			clock_gettime(CLOCK_REALTIME, &now);		
			//			shrinkingtime += (double)(now.tv_sec+now.tv_nsec*1e-9) - (double)(start.tv_sec+start.tv_nsec*1e-9);
			...
			//			clock_gettime(CLOCK_REALTIME, &now);		
			//			updatetime += (double)(now.tv_sec+now.tv_nsec*1e-9) - (double)(start.tv_sec+start.tv_nsec*1e-9);
        ...
        clock_gettime(CLOCK_REALTIME, &now);		
	    totaltime += (double)(now.tv_sec+now.tv_nsec*1e-9) - (double)(start1.tv_sec+start1.tv_nsec*1e-9);

//        if(iter%round == 1){
  			if (1) {
			double iter_time = (double)(now.tv_sec+now.tv_nsec*1e-9) - (double)(start1.tv_sec+start1.tv_nsec*1e-9); 
            double dual_obj = 0;
            ...
            for(int j = 0 ; j < l ; j++){
		        dual_obj += alpha[j]*(alpha[j]*diag[GETI(j)] - 2);
            }
            dual_obj = dual_obj/2;
            info("iter %d walltime %f itertime %f d %f num_upd %d act_sz %d\n", iter, totaltime, iter_time, dual_obj, num_updates_one_iter, active_size);
        }
    ...
    dual_obj = dual_obj/2;
    info("iter %d walltime %f itertime xx d %f num_upd %d act_sz %d\n", iter, totaltime, dual_obj, num_updates_one_iter, active_size);


	info("\ntrainingtime %g\n", totaltime);
	info("xvtime %g\n", xvtime);
	info("shrinkingtime %g\n", shrinkingtime);
	info("updatetime %g\n", updatetime);
	info("total dots %zu\n", total_dots);
	info("inner iter %zu\n", total_inner_iter);
	info("\noptimization finished, #iter = %d\n",iter);
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	switch(param->solver_type)
	{
		...
		case L2R_L2LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL, param->qpsize, param->inner_iter);
			break;
		case L2R_L1LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL, param->qpsize, param->inner_iter);
			break;
		case L1R_L2LOSS_SVC:
		{
			problem prob_col;
